...
pub fn plot_buffer<const N:usize>(buffer: &[f32; N]) {
  ...
  for _ in 0..20000 {
    shapes[0].push(wt1.play::<Hermetic>(buffer, FREQ, 0.0));
    shapes[1].push(wt2.play::<Cosine>  (buffer, FREQ, 0.0));
    shapes[2].push(wt3.play::<Cubic>   (buffer, FREQ, 0.0));
    shapes[3].push(wt4.play::<Linear>  (buffer, FREQ, 0.0));
    shapes[4].push(wt5.play::<Floor>   (buffer, FREQ, 0.0));
  }
