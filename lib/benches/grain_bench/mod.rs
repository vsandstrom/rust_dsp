...
fn grain_new(ng: &mut NewGranulator<32>, buffer: &[f32]) -> [f32; 2] {
  let mut out: [f32; 2] = [0.0,0.0];
  for i in 0..256 {
    out = *ng.play::<Linear, Linear>(buffer);
    if i % 64 == 0 {
      ng.trigger_new(buffer.len(), 0.5, 0.5, 1.0, 0.1, 0.2);
    }
  }
  out
}
...
pub fn criterion_benchmark_grains(c: &mut Criterion) {
  ...
  group.bench_function("ng grains", 
    |b| b.iter(|| {grain_new(&mut g, &buffer)}
  ));
